    ...
    private void submit() {
        ...
        for (IndustrialItem item : items) {
            ...
            if (success) {
                ...
                RewardLogger.logMaintenance(username, item.getDisplayName(), itemType, true);
            } else {
                item.increaseWear(10);
                // Log failed maintenance
                RewardLogger.logMaintenance(username, item.getDisplayName(), itemType, false);
                
                // Remove if critical (destruction will be logged by IndustrialItem)
                if (item.isCritical()) {
